// VEHICULOS
export async function renderVehiculos(el, { supabase, currentUser }) {
  ...
  el.innerHTML = `
    ...
    <div class="tabs">
      ...
      <button class="tab-btn" data-tab="uso">Registrar uso</button>
      <button class="tab-btn" data-tab="historial">Historial</button>
    </div>
    <div class="tab-content active" id="tab-flota"></div>
    <div class="tab-content" id="tab-uso">
      <div style="background:#111;border:1px solid #1e1e1e;padding:20px;border-radius:2px;max-width:420px;">
        <div class="form-row"><label class="form-label">Vehículo <span class="req">*</span></label>
          <select class="form-select" id="uso-veh" id="uso-veh"></select>
        </div>
  ...
  async function loadFlota() {
    ...
    sel.innerHTML = flota.map(v => `<option value="${v.nombre}">${v.nombre}</option>`).join('')
  ...
  el.querySelector('#save-uso').onclick = async () => {
...
// RECEPCION
export async function renderRecepcion(el, { supabase, currentUser }) {
  ...
  el.innerHTML = `
    <div class="page-header">
      ...
      <button class="btn-add" id="btn-new-rec"><i class="ti ti-plus"></i> Nueva recepción</button>
    </div>
    <div class="date-picker-row">
...
export async function renderProductividad(el, { supabase }) {
  el.innerHTML = `
    <div class="page-header"><div class="page-title-group"><span class="page-title">Productividad</span><span class="page-subtitle">Solo jefe de área</span></div></div>
    <div style="display:flex;gap:10px;margin-bottom:20px;flex-wrap:wrap;">
      <select class="filter-select" id="prod-op"><option value="">Todos los operarios</option></select>
      <input type="date" class="date-picker-input" id="prod-desde" placeholder="Desde">
      <input type="date" class="date-picker-input" id="prod-hasta" placeholder="Hasta">
  ...
  async function load() {
    ...
    el.querySelector('#prod-bars').innerHTML = operarios.map((o, i) => {
      ...
      return `<div class="prod-bar-row">
        <div class="prod-bar-label">${o}</div>
        <div class="prod-bar-track"><div class="prod-bar-fill" style="width:${pct}%;background:${colors[i % colors.length]}"></div></div>
        <div class="prod-bar-value">${val} l/min</div>
      </div>`
    }).join('')
    ...
    el.querySelector('#prod-table').innerHTML = `<table class="data-table">
      ...
      <tbody>${lista.map(p => {
        ...
        return `<tr>
          <td>${p.fecha || '—'}</td>
          <td style="font-family:'DM Mono',monospace">${p.nota_pedido}</td>
          <td>${p.cliente_nombre}</td>
          <td style="color:#ccc">${p.operario_arma || '—'}</td>
          <td>${p.operario_controla || '—'}</td>
          <td style="font-family:'DM Mono',monospace">${p.lineas}</td>
          <td style="font-family:'DM Mono',monospace;color:#d4a830">${t}</td>
          <td style="font-family:'DM Mono',monospace;color:#5aadee">${lpm}</td>
          <td style="color:${p.error_count > 0 ? '#ff6b2b' : '#2a2a2a'}">${p.error_count || 0}</td>
        </tr>`}).join('')}
      </tbody></table>`
  }
...
export async function renderUsuarios(el, { supabase }) {
    ...
    <div class="modal-overlay" id="modal-user">
      <div class="modal"><div class="modal-top-bar"></div>
        ...
        <div class="modal-body">
          ...
          <div class="form-row"><label class="form-label">Email <span class="req">*</span></label><input class="form-input" id="u-email" type="email" placeholder="juan@puntobanca.com"></div>
  ...
  const ROLES_LABEL = { jefe: 'Jefe de área', logistica: 'Logística junior', operario: 'Operario', vendedor: 'Vendedor', directivo: 'Directivo' }
  const ROLES_CLASS = { jefe: 'role-jefe', logistica: 'role-logistica', operario: 'role-operario', vendedor: 'role-vendedor', directivo: 'role-directivo' }
  ...
  async function load() {
    ...
    wrap.innerHTML = `<table class="users-table">
      ...
      <tbody>${users.map(u => `<tr>
        ...
        <td><span class="role-tag ${ROLES_CLASS[u.rol]}">${ROLES_LABEL[u.rol]}</span></td>
        ...
        <td style="display:flex;gap:6px">
          ${u.rol !== 'jefe' ? `
            <select class="role-select" data-uid="${u.id}">
              ${['logistica','operario','vendedor','directivo'].map(r => `<option value="${r}" ${u.rol === r ? 'selected' : ''}>${ROLES_LABEL[r]}</option>`).join('')}
            </select>
    ...
    wrap.querySelectorAll('.role-select').forEach(sel => {
      sel.onchange = async () => {
        await supabase.from('profiles').update({ rol: sel.value }).eq('id', sel.dataset.uid)
      }
    })
