...
const { data: allPicking } = await supabase
    ...
    const { data: todosTransportes } = await supabase.from('transportes').select('id, nombre')
    ...
    ;(allPicking || []).forEach(p => {
      ...
      if (tipo === 'retira') {
        ...
        entregasPendientes.push({ ...p, _label: tipo === 'pyb' ? 'Entrega P&B' : (transportesRetiraMap[cliente.transporte_id] || transportesMap[cliente.transporte_id]?.nombre || 'Transp. ext.'), _color: '#a78bfa', _border: '#2d1a52' })
      }
    })
