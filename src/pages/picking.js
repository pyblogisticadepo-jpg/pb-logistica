export async function renderPicking(el, { supabase, currentUser }) {
  el.innerHTML = `
    <div class="page-header">
      <div class="page-title-group"><span class="page-title">Picking</span><span class="page-subtitle" id="picking-count"></span></div>
      <button class="btn-add" id="btn-new-picking"><i class="ti ti-plus"></i> Nuevo picking</button>
    </div>
  ...
  function renderTable(lista) {
    ...
    wrap.innerHTML = `<table class="data-table">
      <thead><tr><th>Nota</th><th>Cliente</th><th>Estado</th><th>Documentación</th><th>Líneas</th><th>Operario arma</th><th>Fecha</th><th></th></tr></thead>
      <tbody>${lista.map(p => `<tr>
        ...
        <td style="font-size:11px;color:#444">${new Date(p.hora_registro).toLocaleDateString('es-AR')}</td>
        <td><button class="btn-sm primary" data-detail="${p.id}"><i class="ti ti-eye"></i></button></td>
      </tr>`).join('')}
      </tbody></table>`
